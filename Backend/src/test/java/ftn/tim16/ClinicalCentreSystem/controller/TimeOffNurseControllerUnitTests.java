...
public class TimeOffNurseControllerUnitTests {
    ...
    public void testApproveRequestForHolidayOrTimeOff() throws Exception {
        ...
        RequestForTimeOffDTO requestForTimeOffDTO = new RequestForTimeOffDTO(ID, "HOLIDAY", new DateTimeInterval(startDate, endDate), NEW_NURSE_FIRST_NAME, NEW_NURSE_lAST_NAME,
                AWAITING);
    ...
    public void testRejectRequestForHolidayOrTimeOff() throws Exception {
        ...
        Mockito.when(timeOffNurseServiceMocked.rejectRequestForHolidayOrTimeOff(ID, reason)).thenReturn(requestForTimeOffDTOSaved);
        ...
        verify(timeOffNurseServiceMocked, times(1)).rejectRequestForHolidayOrTimeOff(ID, reason);
    }

    @Test
    public void testRejectRequestForHolidayOrTimeOff_badRequest() throws Exception {
        String reason = TestUtil.json("At that time we have a lot of work so you have to work.");
        Mockito.when(timeOffNurseServiceMocked.rejectRequestForHolidayOrTimeOff(APPROVED_TIME_OFF, reason)).thenReturn(null);
        ...
        verify(timeOffNurseServiceMocked, times(1)).rejectRequestForHolidayOrTimeOff(APPROVED_TIME_OFF, reason);
    }
